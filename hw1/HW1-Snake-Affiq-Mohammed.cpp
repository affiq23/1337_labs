...
02/05/23: implemented functionality so when snake runs into a wall, it quits game;
          ...
          added comments describing what every major function and code execution in the program is doing
*/

//setting up global variables to use
#include <iostream>
#include <ncurses.h>
using namespace std;
bool gameOver;
...
void drawSnakeOnBoard(){
    ...
    cout << "If game freezes, control+c" << "\r" << endl;
}
...
void snakeLogic(){
    ...
    if ((snakeHeadXPos > width || snakeHeadXPos < 0) || (snakeHeadYPos > height || snakeHeadYPos < 0)){ //if snake runs into left or right wall
        gameOver = true;
    }
    if (snakeHeadXPos >= width){ //if snake runs into top wall
        snakeHeadXPos = 0;
        gameOver = true;
    } 
    ...
    if (snakeHeadYPos >= width){ //if snake runs into botton wall
        snakeHeadYPos = 0;
        gameOver = true;
    }
...
void playGame(){
    ...
    //loop that runs until gameOver is true
    while (!gameOver){
        drawSnakeOnBoard();
        userInput();
        snakeLogic();
    ...
    checkTail(); //tells user if they died due to running into their tail or not
}
